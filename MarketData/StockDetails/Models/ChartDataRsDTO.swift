...
struct ChartDataRsDTO: Codable {
    ...
    var chartDataPoints: [ChartDataPoint] {
        ...
        for string in chartDataPointStrings {
            ...
            chartDataPoints.append(ChartDataPoint(timerstamp: data[0], open: Double(data[1]) ?? 0.0, high: Double(data[2]) ?? 0.0, low: Double(data[3]) ?? 0.0, close: Double(data[4]) ?? 0.0, volume: Double(data[5]) ?? 0.0))
        }
